                      ...
                      off_t offset, struct fuse_file_info *fi)
{
    ...
    // correcteur : [ENCAPS_VIOLATION] vous n'avez pas à faire ça ici, vous ne devriez utiliser que la couche répertoires. direntv6_opendir fait déjà cette vérification, en plus
    struct inode i;
                   ...
                   struct fuse_file_info *fi)
{
    ...
    // correcteur : [ENCAPS_VIOLATION] pas intérêt de faire ça ici, et vous violez les couches. filev6_open fait de toute façon déjà un inode_read et renvoie une erreur si l'appel ne fonctionne pas
    ...
    if (size < SECTOR_SIZE) {
        // correcteur : [USELESS_DYNALLOC] allocation dynamique inutile, un tableau statique aurait très bien suffi
        ...
        if (err < 0) {
            // correcteur : [MEMLEAK] vous devez free(buf2) ici
            return 0;
        } else {
