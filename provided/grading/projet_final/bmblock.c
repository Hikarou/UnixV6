...
struct bmblock_array *bm_alloc(uint64_t min, uint64_t max)
{
    ...
    if (max < min) {
        ...
        // correcteur : la taille allouée n'est pas correcte
        // correcteur : de plus "sizeof(uint64_t)*8" devrait être BITS_PER_VECTOR (même si en l'occurrence ça vaut la même chose)
