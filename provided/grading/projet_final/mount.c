            ...
            if (!err) { // si pas d'erreur de lecture
                if ((actu >= u -> ibm -> min) && (actu <= u -> ibm -> max)) {
                    if (sect_inode[k].i_mode & IALLOC) {
                        ...
                        // correcteur : inutile, le bit correspondant est déjà à 0
                        bm_clear(u -> ibm, actu);
                    }
                }
...
void fill_fbm(struct unix_filesystem * u)
{
    ...
    // correcteur : ça ferait probablement plus de sens de laisser cette responsabilité à bmblock, mais OK
...
int mountv6(const char *filename, struct unix_filesystem *u)
{
    ...
    // correcteur : les deux lignes qui suivent sont inutiles
    u -> fbm = NULL;
    u -> ibm = NULL;
...
int mountv6_mkfs(const char *filename, uint16_t num_blocks, uint16_t num_inodes)
{
    ...
    // correcteur : la vérification demandée était 's.s_fsize < s.s_isize + num_inodes'
    ...
    // correcteur : s_isize est le résultat d'une division entière, donc l'arrondi est fait en dessous.
    // exemple: (21 inodes / 10 inodes per sector) donnerait flloor(2.1) = 2, alors qu'il faudrait 3 secteurs
    s.s_isize = s_isize;
    s.s_fsize = s_fsize;
    ...
    if (err) {
        fclose(fichier); // correcteur : très bien
        return err;
    }
