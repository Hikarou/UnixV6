...
// correcteur : quand vous initialisez un tableau comme ça, vous pouvez omettre la taille (shell_cmds[] = {...} )
...
/* correcteur : [COPYPASTE] toutes vos fonctions do_ ont besoin d'afficher des messages d'erreur correspondant à des codes d'erreur, donc il serait mieux que la fonction 'main' s'en charge plutôt que d'avoir copié coller le code 
...
int main()
{
    ...
    while ((!feof(stdin) && !ferror(stdin)) && err != EXIT) {
        ...
        k = 0;
        err = 0;
        // correcteur [WRONG_EXECUTION] : le sizeof(void*) n'a pas sa place ici, qu'est-il censé représenter ? Une longueur initiale ? Alors elle devrait être dans une constante
        parsed = calloc(NB_ARGS, sizeof(void*));
        if (parsed != NULL) {
            fgets(input, MAX_READ, stdin);
            if (input[strlen(input)-1] == '\n') {
                // correcteur : [DUPLICATED_CALL] stockez le retour de strlen(input) plutôt que de faire deux appels redondants
            ...
            if (feof(stdin) || ferror(stdin)) {
                ...
                if (err == ERR_OK) {
                    ...
                    do {
                        ...
                        ++k;
                        // correcteur : [MAGIC_NUMBER] utilisez NUM_COMMANDS plutôt que 13 !
                    } while (k < 13 && err != 0);
                    --k;
                    if (err == 0) {
...
// correcteur : [S] aucun commentaire et noms de variables peu explicites
...
{
    // correcteur : [MISSING_UMOUNT] il faut penser a umountv6 si un système de fichiers est déjà monté
...
int do_cat(char** args)
{
    ...
    // correcteur : [DUPLICATED_CALL] pas besoin de faire un inode read, il est redondant avec votre filev6_open qui va déjà faire un inode_read. Il serait mieux d'appeler ici filev6_open et de ne pas faire d'inode_read inutile
...
int do_sha(char** args)
{
    ...
    // Allocation test already done in inode_read
    // correcteur : vérification inutile car déjà faite dans print_sha_inode
    if (file.i_node.i_mode & IFDIR) {
...
int do_istat(char** args)
{
    ...
    // correcteur : OK, mais la fonction atoi serait plus appropriée
