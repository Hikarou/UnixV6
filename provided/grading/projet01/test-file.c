/**
 * @file test-file.c
 * @brief program to perform tests on files
 *
 * @date mars 2017
 */

#include <string.h>
#include "mount.h"
#include "error.h"
#include "filev6.h"
#include "inode.h"
#include "sha.h"

//Helper to print the inode
void printTheInode(const struct unix_filesystem *u, uint16_t inr, struct filev6 *f)
{
    int error = filev6_open(u, inr, f);

    if (error >= 0) {
        printf("Printing inode #%u:\n", inr);

        // correcteur : [DUPLICATED_CALL] inutile, filev6_open fait déjà un inode_read. Vous pouvez récupérer l'inode dans f.i_node
        struct inode i;
        error = inode_read(u, inr, &i);

        inode_print(&i);
        if (i.i_mode & IFDIR) {
            printf("which is a directory.\n\n");
        } else {
            uint8_t table[SECTOR_SIZE+1];
            printf("The first sector of data of which contains:\n");
            error = filev6_readblock(f, table);
            table[SECTOR_SIZE] = '\0';
            // correcteur : [BAD_RESULT] il ne faudrait pas afficher le contenu si la lecture a échoué
            printf("%s\n", table);
        }
    } else {
        printf("filve6_open failed for inode #%u\n", inr);
    }
}

int test(struct unix_filesystem *u)
{
    int err = 0;
    struct filev6 f;
    memset(&f, 255, sizeof(f));
    printf("\n");
    printTheInode(u, 3, &f);
    printTheInode(u, 5, &f);
    printf("----\n\nListing inodes SHA:\n");

    uint16_t count = 1;


    // correcteur : il aurait été bien mieux d'utiliser un sector_read pour lire plusieurs inodes d'un coup, autrement votre inode_read va à chaque itération faire un sector_read
    struct inode i;
    err = inode_read(u, count, &i);

    while(err == 0 || (u -> s.s_isize) * INODES_PER_SECTOR < count) {
        print_sha_inode(u, i, count);
        ++count;
        err = inode_read(u, count, &i);
    }
    if (err == ERR_UNALLOCATED_INODE && count > 1) {
        /* puisque le signal d'arrêt est une errur, on remet à zéro si il a bien
         * fait le travail au moins une fois
         */
        err = 0;
    }

    return err;
}
