...
void print_sha_inode(struct unix_filesystem *u, struct inode inode, int inr)
{
    ...
    if (inode.i_mode & IFDIR) {
        ...
        // correcteur : [MISSING_CHECK] vous devez vérifier la valeur de retour de filev6_open et retourner une erreur si celle-ci n'est pas 0
        int error = filev6_open(u, (uint16_t) inr, &f);
        //getting all the content from inode
        do {
            // correcteur : [S] le nom de la variable est très perturbant car filev6_readblock retourne une taille lue, pas une erreur !
