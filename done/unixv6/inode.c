	...
	for (int i = 0; i < u -> s.s_isize; ++i)
	 {
	 	...
	 	/* ICI j'appelle directement la fonction sector, qui est deux couches en dessous. Si
	 	on doit utiliser seulement mount, alors il faut créer une foncition mount, qui appelle 
	 	sector_read. ça me paraît bizarre*/
	 	if (!err){
	 		for (int k = 0; k < 16; ++k)
	 		{
